## Отчет по лабораторной работе №8

---

**Задание**

Реализуйте приложение с GUI (приложения-игры допускается делать с использованием TUI-пакетов) по своему варианту.
- Приложение должно быть написано с применением ОО парадигмы
- Исключительные ситуации должны обрабатываться с использованием собственных исключений

**Название приложения**

Система учета задач (ToDo)

<img width="490" height="469" alt="image" src="https://github.com/user-attachments/assets/0636ab7f-32d9-4c22-b25c-fb5d4a551ae8" />

**Описание**

Классы исключений - собственные исключения для ошибок.

сlass View - абстрактный.

class SimpleView(View), lass DateView(View) - отображение задачи без с даты и с датой.

class Task - хранит информацию об одной задачи.

class TaskList - хранилище всех задач, управление списком задач.

class App - управляет графическим интерфейсом и связывает все компоненты.

**Инструкции по запуску**

Проверить версию питона

```
python --version
```

Установить библиотеку appjar 

```
pip install appJar
```

Запустить файл

```
python3 main.py
```

**Краткую справку**

Кнопки управления

Добавить - создает новую задачу

Выполнено - меняет статус выбранной задачи

Удалить - удаляет выбранную задачу

Сменить вид - переключает режим отображения

Статусы задач

[✓] - Выполнено 

[○] - Не выполнено

Режим отображения 

Обычный вид 

1. [✓] Купить хлеб
2. [✓] Сделать дз

Вид с датой

1. [✓] Купить хлеб (14.05)
2. [✓] Сделать дз (14.05)


**Список использованных истчников**

[except и обработка разных типов исключений](https://metanit.com/python/tutorial/2.20.php)

[Генерация исключений и создание своих типов исключений](https://metanit.com/python/tutorial/2.21.php)

[Полиморфизм](https://python-academy.org/ru/guide/polymorphism)


