...
git add
```
Добавляет всё содержимое рабочей директории в индекс (staging area) для последующего коммита.
```
git commit -m "Имя"
```
Берёт все данные, добавленные с помощью git add, и сохраняет их.
```
git push
```
Отправить измененеия.
```
git pull
```
