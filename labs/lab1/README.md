...
Создал вложенный список со значениями роста члена семьи. Вывел с помощью f строки и индексов роста отца. Создал перменную total_height для подсчета суммарного роста. Дальше с помощью цикла for перебрал всех членов семьи, в каждой итерации прибавлял рост по индексу [1] к total_height. Вывел общий рост семьи.
...
1. Создал 3 переменные для хранения времени звучание одно из трех песен. Перебираю все песни с помощью цикла for. В цикле происходит проверка по названию, если название нужное значение song[1](длительность песни) сохраняется в переменные time1, time2, time3. Вывожу сумму.
2. Использовал прямое обращение к элементам словаря по названию для получения длительности. Вывел сумму.
...
git clone <url>
```
Создает локальную копию (клон) удаленного репозитория Git, указанного в предоставленном URL-адресе.
```
git status 
```
Показывает состояния файлов в рабочей директории: какие файлы изменены, какие ожидают коммита.
```
git add .
```
Команда используемая для переноса изменений из рабочего каталога в область индексации (или «индекс»). Это подготавливает изменения, которые будут включены в следующий git commit.
```
git commit -m "Имя"
```
Берёт все данные, добавленные с помощью git add, и сохраняет их в репозиторий.
```
git push
```
Команда используется для загрузки содержимого локального репозитория в удаленный репозиторий.
```
git pull
```
Используется для получения и загрузки из удалённого репозитория Git и немедленного обновления локального репозитория в соответствии с этим контентом.
